...
class Submit(Resource):
    def post(self):
        ...
        # try:
            engine = sqlalchemy.create_engine("mssql+pyodbc://julesd:Password1@avis-fleet-server.database.windows.net/rv_predict?DRIVER={ODBC Driver 17 for SQL Server}",echo=True)
            ...
            submissions = submissions.append({'ID':max(submissions['ID'] + 1),'Name':name, 'email':email, 'score':score}, ignore_index=True)
            # print('appended')
